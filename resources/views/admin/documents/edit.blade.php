    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.documents.update", [$document->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $document->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                <label for="from_workspace_id">{{ trans('cruds.document.fields.from_workspace') }}</label>
                <select class="form-control select2 {{ $errors->has('from_workspace') ? 'is-invalid' : '' }}" name="from_workspace_id" id="from_workspace_id">
                    @foreach($from_workspaces as $id => $entry)
                        <option value="{{ $id }}" {{ (old('from_workspace_id') ? old('from_workspace_id') : $document->from_workspace->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('from_workspace'))
                    <div class="invalid-feedback">
                        {{ $errors->first('from_workspace') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.document.fields.from_workspace_helper') }}</span>
            </div>
            <div class="form-group">
