...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.analysisItem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.analysis-items.update", [$analysisItem->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="analysis_results">{{ trans('cruds.analysisItem.fields.analysis_results') }}</label>
                <textarea class="form-control {{ $errors->has('analysis_results') ? 'is-invalid' : '' }}" name="analysis_results" id="analysis_results">{{ old('analysis_results', $analysisItem->analysis_results) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('last_analysis_date') ? 'is-invalid' : '' }}" type="text" name="last_analysis_date" id="last_analysis_date" value="{{ old('last_analysis_date', $analysisItem->last_analysis_date) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('comments') ? 'is-invalid' : '' }}" name="comments" id="comments">{{ old('comments', $analysisItem->comments) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('submitter_email') ? 'is-invalid' : '' }}" type="email" name="submitter_email" id="submitter_email" value="{{ old('submitter_email', $analysisItem->submitter_email) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('folder') ? 'is-invalid' : '' }}" name="folder_id" id="folder_id" required>
                    @foreach($folders as $id => $entry)
                        <option value="{{ $id }}" {{ (old('folder_id') ? old('folder_id') : $analysisItem->folder->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('document') ? 'is-invalid' : '' }}" name="document_id" id="document_id">
                    @foreach($documents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('document_id') ? old('document_id') : $analysisItem->document->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('submitter_fullname') ? 'is-invalid' : '' }}" type="text" name="submitter_fullname" id="submitter_fullname" value="{{ old('submitter_fullname', $analysisItem->submitter_fullname) }}">
